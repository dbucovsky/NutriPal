...
**Pages so far** (a simple tab switcher in `App.jsx`, no router library yet): Food, Heart Rate, Sleep, Exercise, Sync. The selected date is lifted to `App.jsx` and shared across the first four tabs (switching tabs keeps the same day) via `frontend/src/components/DateNav.jsx` + `frontend/src/dateUtils.js` (local-day-safe date math, and converting the API's UTC datetime strings to the viewer's local time for display) — `DateNav` shows a "Weekday DD-MMM-YYYY" button that opens the browser's native date picker against a visually-hidden real `<input type="date">`. Heart Rate uses `chart.js`/`react-chartjs-2` (the frontend's first real dependency beyond React itself) for a day's bpm trend — the API pre-buckets raw readings into 10-minute averages server-side (`public/api/heart-rate.php`) rather than shipping tens of thousands of raw points to the browser. Sync (`frontend/src/components/Sync.jsx`, no date — see `doc/wiki/Data-Sync.md`) triggers `sync-google-health.php` and `import-health-connect.php` synchronously and shows their captured output.
