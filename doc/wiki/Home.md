...
**Frontend**: started — `frontend/` (Vite + React) has six real screens (Food, Heart Rate, Sleep, Exercise, Weight, Steps), backed by their own endpoints under `public/api/`. All six support Day/Week/Month/Year/Custom views with collapsible per-day (and, for Food, per-meal) sections, not just a single day. Sync, Settings, Help, and About all live in a dropdown menu on the top ribbon now rather than as tabs; a one-click quick-sync icon sits next to it. See [[Architecture]] for how to run it. Still a placeholder (no real security) login, and no way yet to log/edit anything, only browse — most of the app's actual UI doesn't exist yet.
