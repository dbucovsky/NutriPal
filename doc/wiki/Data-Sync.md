...
**`food_log_entries` cross-source duplication is unresolved and structurally harder.** 13,094 food-log pairs match on `(user_id, start_time)` alone, but only 122 also match on `food_id` — the same real meal usually resolves to a *different* `food_id` between sources (Health Connect's placeholder-serving-unit version vs. the live API's real-gram version), so a naive natural-key dedup ignoring `food_id` isn't safe here the way it is for sleep/exercise/measurements. A real fix needs the "retroactively reconcile Health-Connect placeholder units against real-gram live-API data" work below done first, so the same real food resolves to the same `food_id` regardless of source. Deferred as a separate task.
...
Fixed the display problem specifically: verified the real numeric `ExerciseSessionType` values against Android's own documentation one at a time (not guessed — a wrong label would be worse than the honest `HC_<code>` fallback already in place) and added a translation table for the codes that actually appear in this account's real data — `53`→Walking, `34`→Running (Treadmill), `33`→Running, `60`→Elliptical, `49`→Swimming (Pool), `59`→Stair Climbing (Machine), `4`→Biking — covering 1,992 of 2,014 real exercise sessions. Two rarer codes (`58`, `11`) stay as the honest `HC_<code>` fallback rather than a guessed name. Note: the live API's own `exerciseType` values (confirmed real examples: `WALKING`, `OUTDOOR_BIKE`, `STAIRCLIMBER`, `WORKOUT`) turned out to be a genuinely *different* vocabulary from Health Connect's numeric codes, not just a differently-formatted version of the same one — unifying both into one canonical `lut_activity_type` space is a real, larger effort, tracked as an open item below, not attempted here.
...
- Two Health Connect `exercise_type` codes seen in this account's real data (`58`, `11`) aren't in the `HC_EXERCISE_TYPE_NAMES` translation table yet (deliberately not guessed) — they still display as `HC_58`/`HC_11` in the UI. Add them once their real Android `ExerciseSessionType` values are confirmed.
