...
Fixed with real natural-key constraints — `UNIQUE(user_id, start_time)` on `sleep_sessions`/`exercise_sessions`, `UNIQUE(user_id, measurement_type_id, reading_time)` on `measurements` — plus a new cross-source check in `sync-google-health.php`'s generic `upsertByNaturalKey()`: if the primary `(user_id, api_uid)` lookup misses but the natural-key lookup hits, the row already exists under the other source's `api_uid`, so it's skipped rather than inserted (would duplicate) or updated (would silently overwrite that source's provenance, and for measurements, its more-precise value). `import-health-connect.php`'s sleep-session dedup lookup switched from `api_uid` to `start_time` for the same reason.
...
Verified against the real database: rebuilt, re-imported the HC export, ran a real `--full` live sync (`SLEEP_SESSIONS: inserted=552, skipped_cross_source=660`, `EXERCISE: inserted=291, skipped_cross_source=1979`, `WEIGHT: inserted=6, skipped_cross_source=513` — matching the pre-fix analysis almost exactly), confirmed zero internal duplication on all three tables via direct query, and confirmed a second HC re-import stays fully idempotent.
